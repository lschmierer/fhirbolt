...
impl Resource {
    pub fn id(&self) -> Option<&str> {
        match self {
            ...
            Resource::CatalogEntry(r) => r.id.as_deref(),
            ...
            Resource::DeviceUseStatement(r) => r.id.as_deref(),
            Resource::DiagnosticReport(r) => r.id.as_deref(),
            Resource::DocumentManifest(r) => r.id.as_deref(),
            Resource::DocumentReference(r) => r.id.as_deref(),
            Resource::EffectEvidenceSynthesis(r) => r.id.as_deref(),
            ...
            Resource::Media(r) => r.id.as_deref(),
            ...
            Resource::MedicinalProduct(r) => r.id.as_deref(),
            Resource::MedicinalProductAuthorization(r) => r.id.as_deref(),
            Resource::MedicinalProductContraindication(r) => r.id.as_deref(),
            Resource::MedicinalProductIndication(r) => r.id.as_deref(),
            Resource::MedicinalProductIngredient(r) => r.id.as_deref(),
            Resource::MedicinalProductInteraction(r) => r.id.as_deref(),
            Resource::MedicinalProductManufactured(r) => r.id.as_deref(),
            Resource::MedicinalProductPackaged(r) => r.id.as_deref(),
            Resource::MedicinalProductPharmaceutical(r) => r.id.as_deref(),
            Resource::MedicinalProductUndesirableEffect(r) => r.id.as_deref(),
            ...
            Resource::RequestGroup(r) => r.id.as_deref(),
            Resource::ResearchDefinition(r) => r.id.as_deref(),
            Resource::ResearchElementDefinition(r) => r.id.as_deref(),
            Resource::ResearchStudy(r) => r.id.as_deref(),
            Resource::ResearchSubject(r) => r.id.as_deref(),
            Resource::RiskAssessment(r) => r.id.as_deref(),
            Resource::RiskEvidenceSynthesis(r) => r.id.as_deref(),
            ...
            Resource::SubstanceSpecification(r) => r.id.as_deref(),
    ...
    pub fn meta(&self) -> Option<&Box<crate::r4::types::Meta>> {
        match self {
            Resource::Account(r) => r.meta.as_ref(),
            Resource::ActivityDefinition(r) => r.meta.as_ref(),
            Resource::AdverseEvent(r) => r.meta.as_ref(),
            Resource::AllergyIntolerance(r) => r.meta.as_ref(),
            Resource::Appointment(r) => r.meta.as_ref(),
            Resource::AppointmentResponse(r) => r.meta.as_ref(),
            Resource::AuditEvent(r) => r.meta.as_ref(),
            Resource::Basic(r) => r.meta.as_ref(),
            Resource::Binary(r) => r.meta.as_ref(),
            Resource::BiologicallyDerivedProduct(r) => r.meta.as_ref(),
            Resource::BodyStructure(r) => r.meta.as_ref(),
            Resource::Bundle(r) => r.meta.as_ref(),
            Resource::CapabilityStatement(r) => r.meta.as_ref(),
            Resource::CarePlan(r) => r.meta.as_ref(),
            Resource::CareTeam(r) => r.meta.as_ref(),
            Resource::CatalogEntry(r) => r.meta.as_ref(),
            Resource::ChargeItem(r) => r.meta.as_ref(),
            Resource::ChargeItemDefinition(r) => r.meta.as_ref(),
            Resource::Claim(r) => r.meta.as_ref(),
            Resource::ClaimResponse(r) => r.meta.as_ref(),
            Resource::ClinicalImpression(r) => r.meta.as_ref(),
            Resource::CodeSystem(r) => r.meta.as_ref(),
            Resource::Communication(r) => r.meta.as_ref(),
            Resource::CommunicationRequest(r) => r.meta.as_ref(),
            Resource::CompartmentDefinition(r) => r.meta.as_ref(),
            Resource::Composition(r) => r.meta.as_ref(),
            Resource::ConceptMap(r) => r.meta.as_ref(),
            Resource::Condition(r) => r.meta.as_ref(),
            Resource::Consent(r) => r.meta.as_ref(),
            Resource::Contract(r) => r.meta.as_ref(),
            Resource::Coverage(r) => r.meta.as_ref(),
            Resource::CoverageEligibilityRequest(r) => r.meta.as_ref(),
            Resource::CoverageEligibilityResponse(r) => r.meta.as_ref(),
            Resource::DetectedIssue(r) => r.meta.as_ref(),
            Resource::Device(r) => r.meta.as_ref(),
            Resource::DeviceDefinition(r) => r.meta.as_ref(),
            Resource::DeviceMetric(r) => r.meta.as_ref(),
            Resource::DeviceRequest(r) => r.meta.as_ref(),
            Resource::DeviceUseStatement(r) => r.meta.as_ref(),
            Resource::DiagnosticReport(r) => r.meta.as_ref(),
            Resource::DocumentManifest(r) => r.meta.as_ref(),
            Resource::DocumentReference(r) => r.meta.as_ref(),
            Resource::EffectEvidenceSynthesis(r) => r.meta.as_ref(),
            Resource::Encounter(r) => r.meta.as_ref(),
            Resource::Endpoint(r) => r.meta.as_ref(),
            Resource::EnrollmentRequest(r) => r.meta.as_ref(),
            Resource::EnrollmentResponse(r) => r.meta.as_ref(),
            Resource::EpisodeOfCare(r) => r.meta.as_ref(),
            Resource::EventDefinition(r) => r.meta.as_ref(),
            Resource::Evidence(r) => r.meta.as_ref(),
            Resource::EvidenceVariable(r) => r.meta.as_ref(),
            Resource::ExampleScenario(r) => r.meta.as_ref(),
            Resource::ExplanationOfBenefit(r) => r.meta.as_ref(),
            Resource::FamilyMemberHistory(r) => r.meta.as_ref(),
            Resource::Flag(r) => r.meta.as_ref(),
            Resource::Goal(r) => r.meta.as_ref(),
            Resource::GraphDefinition(r) => r.meta.as_ref(),
            Resource::Group(r) => r.meta.as_ref(),
            Resource::GuidanceResponse(r) => r.meta.as_ref(),
            Resource::HealthcareService(r) => r.meta.as_ref(),
            Resource::ImagingStudy(r) => r.meta.as_ref(),
            Resource::Immunization(r) => r.meta.as_ref(),
            Resource::ImmunizationEvaluation(r) => r.meta.as_ref(),
            Resource::ImmunizationRecommendation(r) => r.meta.as_ref(),
            Resource::ImplementationGuide(r) => r.meta.as_ref(),
            Resource::InsurancePlan(r) => r.meta.as_ref(),
            Resource::Invoice(r) => r.meta.as_ref(),
            Resource::Library(r) => r.meta.as_ref(),
            Resource::Linkage(r) => r.meta.as_ref(),
            Resource::List(r) => r.meta.as_ref(),
            Resource::Location(r) => r.meta.as_ref(),
            Resource::Measure(r) => r.meta.as_ref(),
            Resource::MeasureReport(r) => r.meta.as_ref(),
            Resource::Media(r) => r.meta.as_ref(),
            Resource::Medication(r) => r.meta.as_ref(),
            Resource::MedicationAdministration(r) => r.meta.as_ref(),
            Resource::MedicationDispense(r) => r.meta.as_ref(),
            Resource::MedicationKnowledge(r) => r.meta.as_ref(),
            Resource::MedicationRequest(r) => r.meta.as_ref(),
            Resource::MedicationStatement(r) => r.meta.as_ref(),
            Resource::MedicinalProduct(r) => r.meta.as_ref(),
            Resource::MedicinalProductAuthorization(r) => r.meta.as_ref(),
            Resource::MedicinalProductContraindication(r) => r.meta.as_ref(),
            Resource::MedicinalProductIndication(r) => r.meta.as_ref(),
            Resource::MedicinalProductIngredient(r) => r.meta.as_ref(),
            Resource::MedicinalProductInteraction(r) => r.meta.as_ref(),
            Resource::MedicinalProductManufactured(r) => r.meta.as_ref(),
            Resource::MedicinalProductPackaged(r) => r.meta.as_ref(),
            Resource::MedicinalProductPharmaceutical(r) => r.meta.as_ref(),
            Resource::MedicinalProductUndesirableEffect(r) => r.meta.as_ref(),
            Resource::MessageDefinition(r) => r.meta.as_ref(),
            Resource::MessageHeader(r) => r.meta.as_ref(),
            Resource::MolecularSequence(r) => r.meta.as_ref(),
            Resource::NamingSystem(r) => r.meta.as_ref(),
            Resource::NutritionOrder(r) => r.meta.as_ref(),
            Resource::Observation(r) => r.meta.as_ref(),
            Resource::ObservationDefinition(r) => r.meta.as_ref(),
            Resource::OperationDefinition(r) => r.meta.as_ref(),
            Resource::OperationOutcome(r) => r.meta.as_ref(),
            Resource::Organization(r) => r.meta.as_ref(),
            Resource::OrganizationAffiliation(r) => r.meta.as_ref(),
            Resource::Parameters(r) => r.meta.as_ref(),
            Resource::Patient(r) => r.meta.as_ref(),
            Resource::PaymentNotice(r) => r.meta.as_ref(),
            Resource::PaymentReconciliation(r) => r.meta.as_ref(),
            Resource::Person(r) => r.meta.as_ref(),
            Resource::PlanDefinition(r) => r.meta.as_ref(),
            Resource::Practitioner(r) => r.meta.as_ref(),
            Resource::PractitionerRole(r) => r.meta.as_ref(),
            Resource::Procedure(r) => r.meta.as_ref(),
            Resource::Provenance(r) => r.meta.as_ref(),
            Resource::Questionnaire(r) => r.meta.as_ref(),
            Resource::QuestionnaireResponse(r) => r.meta.as_ref(),
            Resource::RelatedPerson(r) => r.meta.as_ref(),
            Resource::RequestGroup(r) => r.meta.as_ref(),
            Resource::ResearchDefinition(r) => r.meta.as_ref(),
            Resource::ResearchElementDefinition(r) => r.meta.as_ref(),
            Resource::ResearchStudy(r) => r.meta.as_ref(),
            Resource::ResearchSubject(r) => r.meta.as_ref(),
            Resource::RiskAssessment(r) => r.meta.as_ref(),
            Resource::RiskEvidenceSynthesis(r) => r.meta.as_ref(),
            Resource::Schedule(r) => r.meta.as_ref(),
            Resource::SearchParameter(r) => r.meta.as_ref(),
            Resource::ServiceRequest(r) => r.meta.as_ref(),
            Resource::Slot(r) => r.meta.as_ref(),
            Resource::Specimen(r) => r.meta.as_ref(),
            Resource::SpecimenDefinition(r) => r.meta.as_ref(),
            Resource::StructureDefinition(r) => r.meta.as_ref(),
            Resource::StructureMap(r) => r.meta.as_ref(),
            Resource::Subscription(r) => r.meta.as_ref(),
            Resource::Substance(r) => r.meta.as_ref(),
            Resource::SubstanceNucleicAcid(r) => r.meta.as_ref(),
            Resource::SubstancePolymer(r) => r.meta.as_ref(),
            Resource::SubstanceProtein(r) => r.meta.as_ref(),
            Resource::SubstanceReferenceInformation(r) => r.meta.as_ref(),
            Resource::SubstanceSourceMaterial(r) => r.meta.as_ref(),
            Resource::SubstanceSpecification(r) => r.meta.as_ref(),
            Resource::SupplyDelivery(r) => r.meta.as_ref(),
            Resource::SupplyRequest(r) => r.meta.as_ref(),
            Resource::Task(r) => r.meta.as_ref(),
            Resource::TerminologyCapabilities(r) => r.meta.as_ref(),
            Resource::TestReport(r) => r.meta.as_ref(),
            Resource::TestScript(r) => r.meta.as_ref(),
            Resource::ValueSet(r) => r.meta.as_ref(),
            Resource::VerificationResult(r) => r.meta.as_ref(),
            Resource::VisionPrescription(r) => r.meta.as_ref(),
            _ => None,
        }
    }
}
