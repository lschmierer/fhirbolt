...
use fhirbolt_model::r4b::types::Money;
impl serde::ser::Serialize for SerializationContext<&Money> {
        ...
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#currency.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("currency", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    use super::super::serde_helpers::PrimitiveElement;
                    let primitive_element = PrimitiveElement {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_currency", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#currency.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("currency", ctx)));
        }
        state.end()
    }
}
impl serde::ser::Serialize for SerializationContext<&Box<Money>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl serde::ser::Serialize for SerializationContext<&Vec<Money>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl<'de> serde::de::DeserializeSeed<'de> for &mut DeserializationContext<Money> {
    type Value = Money;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<Money>);
...
impl<'de> serde::de::DeserializeSeed<'de> for &mut DeserializationContext<Box<Money>> {
    type Value = Box<Money>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        self.transmute::<Money>()
            .deserialize(deserializer)
            .map(Box::new)
    }
}
impl<'de> serde::de::DeserializeSeed<'de> for &mut DeserializationContext<Vec<Money>> {
    type Value = Vec<Money>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<Vec<Money>>);
        impl<'de> serde::de::Visitor<'de> for Visitor<'_> {
            type Value = Vec<Money>;
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                ...
                A: serde::de::SeqAccess<'de>,
            {
                ...
                let _context: &mut DeserializationContext<Money> = self.0.transmute();
