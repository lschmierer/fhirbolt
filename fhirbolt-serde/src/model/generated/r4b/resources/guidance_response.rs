...
impl serde::ser::Serialize for SerializationContext<&GuidanceResponse> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        {
            use fhirbolt_model::r4b::resources::GuidanceResponseModule as _Enum;
        ...
        if !self.value.r#evaluation_message.is_empty() {
            tri!(
                self.with_context(&self.value.r#evaluation_message, |ctx| state
                    .serialize_entry("evaluationMessage", ctx))
            );
        }
        ...
        if let Some(some) = self.value.r#result.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("result", ctx)));
        }
