...
use fhirbolt_model::r4b::resources::EnrollmentRequest;
...
impl serde::ser::Serialize for SerializationContext<&EnrollmentRequest> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        tri!(state.serialize_entry("resourceType", "EnrollmentRequest"));
        ...
        if let Some(some) = self.value.r#candidate.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("candidate", ctx)));
        }
        if let Some(some) = self.value.r#coverage.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("coverage", ctx)));
        }
        state.end()
    }
}
                    ...
                    match map_access_key {
                        ...
                        Field::Candidate => {
                            ...
                            let _context: &mut DeserializationContext<
                                Box<fhirbolt_model::r4b::types::Reference>,
                            > = self.0.transmute();
                            r#candidate = Some(tri!(map_access.next_value_seed(&mut *_context)));
                        }
                        Field::Coverage => {
