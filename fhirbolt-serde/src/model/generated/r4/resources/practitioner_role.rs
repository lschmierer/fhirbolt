...
use fhirbolt_model::r4::resources::PractitionerRoleAvailableTime;
...
use fhirbolt_model::r4::resources::PractitionerRoleNotAvailable;
...
use fhirbolt_model::r4::resources::PractitionerRole;
...
impl serde::ser::Serialize for SerializationContext<&PractitionerRole> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        let mut state = tri!(serializer.serialize_map(None));
        tri!(state.serialize_entry("resourceType", "PractitionerRole"));
        if self.output == crate::context::Format::Json {
        ...
        if let Some(some) = self.value.r#practitioner.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("practitioner", ctx)));
        }
        ...
        if !self.value.r#healthcare_service.is_empty() {
            tri!(
                self.with_context(&self.value.r#healthcare_service, |ctx| state
                    .serialize_entry("healthcareService", ctx))
            );
        }
                ...
                while let Some(map_access_key) = tri!(map_access.next_key()) {
                    match map_access_key {
                        ...
                        Field::Practitioner => {
                            ...
                            > = self.0.transmute();
                            r#practitioner = Some(tri!(map_access.next_value_seed(&mut *_context)));
                        }
                        Field::Organization => {
                        ...
                        Field::AvailableTime => {
                            if self.0.from == crate::context::Format::Json {
                                ...
                                let _context : & mut DeserializationContext < Vec < fhirbolt_model :: r4 :: resources :: PractitionerRoleAvailableTime >> = self . 0 . transmute () ;
                                r#available_time =
                                    Some(tri!(map_access.next_value_seed(&mut *_context)));
                            } else {
