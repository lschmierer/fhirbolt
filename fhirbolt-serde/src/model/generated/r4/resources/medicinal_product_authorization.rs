...
impl serde::ser::Serialize for SerializationContext<&MedicinalProductAuthorization> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        let mut state = tri!(serializer.serialize_map(None));
        tri!(state.serialize_entry("resourceType", "MedicinalProductAuthorization"));
        if self.output == crate::context::Format::Json {
        ...
        if let Some(some) = self.value.r#data_exclusivity_period.as_ref() {
            tri!(self.with_context(some, |ctx| state
                .serialize_entry("dataExclusivityPeriod", ctx)));
        }
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#date_of_first_authorization.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("dateOfFirstAuthorization", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    use super::super::serde_helpers::PrimitiveElement;
                    let primitive_element = PrimitiveElement {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_dateOfFirstAuthorization", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#date_of_first_authorization.as_ref() {
            tri!(self.with_context(some, |ctx| state
                .serialize_entry("dateOfFirstAuthorization", ctx)));
        }
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#international_birth_date.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("internationalBirthDate", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    use super::super::serde_helpers::PrimitiveElement;
                    let primitive_element = PrimitiveElement {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_internationalBirthDate", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#international_birth_date.as_ref() {
            tri!(self.with_context(some, |ctx| state
                .serialize_entry("internationalBirthDate", ctx)));
        }
        if let Some(some) = self.value.r#legal_basis.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("legalBasis", ctx)));
        }
        if !self.value.r#jurisdictional_authorization.is_empty() {
            tri!(
                self.with_context(&self.value.r#jurisdictional_authorization, |ctx| state
                    .serialize_entry("jurisdictionalAuthorization", ctx))
            );
        }
                    ...
                    match map_access_key {
                        ...
                        Field::LegalBasis => {
                            ...
                            let _context: &mut DeserializationContext<
                                Box<fhirbolt_model::r4::types::CodeableConcept>,
                            > = self.0.transmute();
                            r#legal_basis = Some(tri!(map_access.next_value_seed(&mut *_context)));
                        }
