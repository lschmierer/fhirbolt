        ...
        if self.output_json {
            if let Some(some) = self.value.r#assigned_arm.as_ref() {
                ...
                if some.id.is_some() || !some.extension.is_empty() {
                    ...
                    self.with_context(&primitive_element, |ctx| {
                        state.serialize_entry("_assignedArm", ctx)
                    })?;
                }
            }
        } else if let Some(some) = self.value.r#assigned_arm.as_ref() {
            self.with_context(some, |ctx| state.serialize_entry("assignedArm", ctx))?;
        }
        if self.output_json {
            if let Some(some) = self.value.r#actual_arm.as_ref() {
                ...
                if some.id.is_some() || !some.extension.is_empty() {
                    ...
                    self.with_context(&primitive_element, |ctx| {
                        state.serialize_entry("_actualArm", ctx)
                    })?;
                }
            }
        } else if let Some(some) = self.value.r#actual_arm.as_ref() {
            self.with_context(some, |ctx| state.serialize_entry("actualArm", ctx))?;
        }
        if let Some(some) = self.value.r#consent.as_ref() {
            self.with_context(some, |ctx| state.serialize_entry("consent", ctx))?;
        }
        state.end()
    }
}
                    ...
                    match map_access_key {
                        ...
                        Field::AssignedArm => {
                            if self.0.from_json {
                                ...
                                > = self.0.transmute();
                                r#assigned_arm = Some(map_access.next_value_seed(&mut *_context)?);
                            }
                        }
                        ...
                        Field::ActualArm => {
                            if self.0.from_json {
                                ...
                                let _context: &mut DeserializationContext<
                                    fhirbolt_model::r4::types::String,
                                > = self.0.transmute();
                                r#actual_arm = Some(map_access.next_value_seed(&mut *_context)?);
                            }
                        }
