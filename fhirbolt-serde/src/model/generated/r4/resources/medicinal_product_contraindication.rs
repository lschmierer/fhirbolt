...
impl serde::ser::Serialize for SerializationContext<&MedicinalProductContraindication> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        let mut state = tri!(serializer.serialize_map(None));
        tri!(state.serialize_entry("resourceType", "MedicinalProductContraindication"));
        ...
        if let Some(some) = self.value.r#disease.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("disease", ctx)));
        }
        ...
        if !self.value.r#therapeutic_indication.is_empty() {
            tri!(
                self.with_context(&self.value.r#therapeutic_indication, |ctx| state
                    .serialize_entry("therapeuticIndication", ctx))
            );
        }
                    ...
                    match map_access_key {
                        ...
                        Field::Disease => {
                            ...
                            let _context: &mut DeserializationContext<
                                Box<fhirbolt_model::r4::types::CodeableConcept>,
                            > = self.0.transmute();
                            r#disease = Some(tri!(map_access.next_value_seed(&mut *_context)));
                        }
                        Field::DiseaseStatus => {
