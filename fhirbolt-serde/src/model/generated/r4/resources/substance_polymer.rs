    ...
    for SerializationContext<&SubstancePolymerRepeatRepeatUnitDegreeOfPolymerisation>
{
        ...
        S: serde::ser::Serializer,
    {
        ...
        if let Some(some) = self.value.r#degree.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("degree", ctx)));
        }
        if let Some(some) = self.value.r#amount.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("amount", ctx)));
        }
                    ...
                    match map_access_key {
                        ...
                        Field::Degree => {
                            ...
                            let _context: &mut DeserializationContext<
                                Box<fhirbolt_model::r4::types::CodeableConcept>,
                            > = self.0.transmute();
                            r#degree = Some(tri!(map_access.next_value_seed(&mut *_context)));
                        }
                        Field::Amount => {
                            if r#amount.is_some() {
                                return Err(serde::de::Error::duplicate_field("amount"));
        ...
        S: serde::ser::Serializer,
    {
        ...
        if let Some(some) = self.value.r#orientation_of_polymerisation.as_ref() {
            tri!(self.with_context(some, |ctx| state
                .serialize_entry("orientationOfPolymerisation", ctx)));
        }
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#repeat_unit.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("repeatUnit", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    use super::super::serde_helpers::PrimitiveElement;
                    let primitive_element = PrimitiveElement {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_repeatUnit", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#repeat_unit.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("repeatUnit", ctx)));
        }
        if let Some(some) = self.value.r#amount.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("amount", ctx)));
        }
        ...
        S: serde::ser::Serializer,
    {
        ...
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#number_of_units.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("numberOfUnits", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    use super::super::serde_helpers::PrimitiveElement;
                    let primitive_element = PrimitiveElement {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_numberOfUnits", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#number_of_units.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("numberOfUnits", ctx)));
        }
        if self.output == crate::context::Format::Json {
        ...
        if self.output == crate::context::Format::Json {
            if !self.value.r#modification.is_empty() {
                ...
                if values.iter().any(|v| v.is_some()) {
                    tri!(state.serialize_entry("modification", &values));
                }
            ...
            tri!(self.with_context(&self.value.r#modification, |ctx| state
                .serialize_entry("modification", ctx)));
        }
