...
use fhirbolt_model::r5::resources::GraphDefinitionNode;
impl serde::ser::Serialize for SerializationContext<&GraphDefinitionNode> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        if self.output == crate::context::Format::Json {
            if self.value.r#node_id.id.as_deref() == Some("$invalid") {
                return missing_field_error("nodeId");
            }
            if let Some(some) = self.value.r#node_id.value.as_ref().map(Ok) {
                tri!(state.serialize_entry("nodeId", &some?));
            }
            if self.value.r#node_id.id.is_some() || !self.value.r#node_id.extension.is_empty() {
                use super::super::serde_helpers::PrimitiveElement;
                let primitive_element = PrimitiveElement {
                    id: self.value.r#node_id.id.as_ref(),
                    extension: &self.value.r#node_id.extension,
                };
                tri!(self.with_context(&primitive_element, |ctx| state
                    .serialize_entry("_nodeId", ctx)));
            }
        } else if self.value.r#node_id.id.as_deref() == Some("$invalid") {
            return missing_field_error("nodeId");
        } else {
            tri!(self.with_context(&self.value.r#node_id, |ctx| state
                .serialize_entry("nodeId", ctx)));
        }
        if self.output == crate::context::Format::Json {
...
impl serde::ser::Serialize for SerializationContext<&Box<GraphDefinitionNode>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl serde::ser::Serialize for SerializationContext<&Vec<GraphDefinitionNode>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl<'de> serde::de::DeserializeSeed<'de> for &mut DeserializationContext<GraphDefinitionNode> {
    type Value = GraphDefinitionNode;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<GraphDefinitionNode>);
        impl<'de> serde::de::Visitor<'de> for Visitor<'_> {
            type Value = GraphDefinitionNode;
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_str("GraphDefinitionNode")
            }
            fn visit_map<V>(self, mut map_access: V) -> Result<GraphDefinitionNode, V::Error>
            where
                V: serde::de::MapAccess<'de>,
            {
                ...
                let mut r#modifier_extension: Option<Vec<fhirbolt_model::r5::types::Extension>> =
                    None;
                let mut r#node_id: Option<fhirbolt_model::r5::types::Id> = None;
                ...
                while let Some(map_access_key) = tri!(map_access.next_key()) {
                    match map_access_key {
                        ...
                        Field::NodeId => {
                            if self.0.from == crate::context::Format::Json {
                                let some = r#node_id.get_or_insert(Default::default());
                                ...
                                > = self.0.transmute();
                                r#node_id = Some(tri!(map_access.next_value_seed(&mut *_context)));
                            }
                        }
                        Field::NodeIdPrimitiveElement => {
                            if self.0.from == crate::context::Format::Json {
                                let some = r#node_id.get_or_insert(Default::default());
                                if some.id.is_some() || !some.extension.is_empty() {
                                    return Err(serde::de::Error::duplicate_field("_nodeId"));
                                }
                                ...
                                return unknown_field_error("nodeId");
                            }
                        }
                        Field::Description => {
                            if self.0.from == crate::context::Format::Json {
                ...
                Ok(GraphDefinitionNode {
                    r#id,
                    r#extension: r#extension.unwrap_or(vec![]),
                    r#modifier_extension: r#modifier_extension.unwrap_or(vec![]),
                    r#node_id: if self.0.config.mode == crate::context::de::DeserializationMode::Lax
                    {
                        r#node_id.unwrap_or(Default::default())
                    } else {
                        tri!(r#node_id.ok_or(serde::de::Error::missing_field("nodeId")))
                    },
                    r#description,
...
impl<'de> serde::de::DeserializeSeed<'de>
    for &mut DeserializationContext<Box<GraphDefinitionNode>>
{
    type Value = Box<GraphDefinitionNode>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        self.transmute::<GraphDefinitionNode>()
            .deserialize(deserializer)
            .map(Box::new)
    }
}
impl<'de> serde::de::DeserializeSeed<'de>
    for &mut DeserializationContext<Vec<GraphDefinitionNode>>
{
    type Value = Vec<GraphDefinitionNode>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<Vec<GraphDefinitionNode>>);
        impl<'de> serde::de::Visitor<'de> for Visitor<'_> {
            type Value = Vec<GraphDefinitionNode>;
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                ...
                A: serde::de::SeqAccess<'de>,
            {
                ...
                let _context: &mut DeserializationContext<GraphDefinitionNode> = self.0.transmute();
...
use fhirbolt_model::r5::resources::GraphDefinitionLinkCompartment;
impl serde::ser::Serialize for SerializationContext<&GraphDefinitionLinkCompartment> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
        ...
        fn missing_field_error<T, E: serde::ser::Error>(field: &str) -> Result<T, E> {
            Err(E::custom(format!(
                "missing required field `{}.{}`",
                "GraphDefinition.link.compartment", field
            )))
        }
        let mut state = tri!(serializer.serialize_map(None));
        if let Some(value) = self.value.r#id.as_ref() {
        ...
        if self.output == crate::context::Format::Json {
            ...
            if let Some(some) = self.value.r#rule.value.as_ref().map(Ok) {
                tri!(state.serialize_entry("rule", &some?));
            }
            if self.value.r#rule.id.is_some() || !self.value.r#rule.extension.is_empty() {
                use super::super::serde_helpers::PrimitiveElement;
                let primitive_element = PrimitiveElement {
                    id: self.value.r#rule.id.as_ref(),
                    extension: &self.value.r#rule.extension,
                };
                tri!(self.with_context(&primitive_element, |ctx| state
                    .serialize_entry("_rule", ctx)));
            }
...
impl serde::ser::Serialize for SerializationContext<&Box<GraphDefinitionLinkCompartment>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl serde::ser::Serialize for SerializationContext<&Vec<GraphDefinitionLinkCompartment>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl<'de> serde::de::DeserializeSeed<'de>
    for &mut DeserializationContext<GraphDefinitionLinkCompartment>
{
    type Value = GraphDefinitionLinkCompartment;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<GraphDefinitionLinkCompartment>);
        impl<'de> serde::de::Visitor<'de> for Visitor<'_> {
            type Value = GraphDefinitionLinkCompartment;
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_str("GraphDefinitionLinkCompartment")
            }
            fn visit_map<V>(
                self,
                mut map_access: V,
            ) -> Result<GraphDefinitionLinkCompartment, V::Error>
            where
                V: serde::de::MapAccess<'de>,
            {
                #[derive(serde :: Deserialize)]
                ...
                let mut r#rule: Option<fhirbolt_model::r5::types::Code> = None;
                ...
                Ok(GraphDefinitionLinkCompartment {
                    r#id,
                    r#extension: r#extension.unwrap_or(vec![]),
                    r#modifier_extension: r#modifier_extension.unwrap_or(vec![]),
                    ...
                    r#rule: if self.0.config.mode == crate::context::de::DeserializationMode::Lax {
                        r#rule.unwrap_or(Default::default())
                    } else {
                        tri!(r#rule.ok_or(serde::de::Error::missing_field("rule")))
                    },
...
impl<'de> serde::de::DeserializeSeed<'de>
    for &mut DeserializationContext<Box<GraphDefinitionLinkCompartment>>
{
    type Value = Box<GraphDefinitionLinkCompartment>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        self.transmute::<GraphDefinitionLinkCompartment>()
            .deserialize(deserializer)
            .map(Box::new)
    }
}
impl<'de> serde::de::DeserializeSeed<'de>
    for &mut DeserializationContext<Vec<GraphDefinitionLinkCompartment>>
{
    type Value = Vec<GraphDefinitionLinkCompartment>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<Vec<GraphDefinitionLinkCompartment>>);
        impl<'de> serde::de::Visitor<'de> for Visitor<'_> {
            type Value = Vec<GraphDefinitionLinkCompartment>;
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_str("a sequence")
            ...
            {
                let mut values = Vec::new();
                let _context: &mut DeserializationContext<GraphDefinitionLinkCompartment> =
                    self.0.transmute();
                while let Some(value) = tri!(seq.next_element_seed(&mut *_context)) {
                    values.push(value);
                }
...
use fhirbolt_model::r5::resources::GraphDefinitionLink;
impl serde::ser::Serialize for SerializationContext<&GraphDefinitionLink> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
        ...
        if self.output == crate::context::Format::Json {
            if self.value.r#source_id.id.as_deref() == Some("$invalid") {
                return missing_field_error("sourceId");
            }
            if let Some(some) = self.value.r#source_id.value.as_ref().map(Ok) {
                tri!(state.serialize_entry("sourceId", &some?));
            }
            if self.value.r#source_id.id.is_some() || !self.value.r#source_id.extension.is_empty() {
                use super::super::serde_helpers::PrimitiveElement;
                let primitive_element = PrimitiveElement {
                    id: self.value.r#source_id.id.as_ref(),
                    extension: &self.value.r#source_id.extension,
                };
                tri!(self.with_context(&primitive_element, |ctx| state
            ...
            tri!(self.with_context(&self.value.r#source_id, |ctx| state
                .serialize_entry("sourceId", ctx)));
        }
        if self.output == crate::context::Format::Json {
...
impl serde::ser::Serialize for SerializationContext<&Box<GraphDefinitionLink>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl serde::ser::Serialize for SerializationContext<&Vec<GraphDefinitionLink>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl<'de> serde::de::DeserializeSeed<'de> for &mut DeserializationContext<GraphDefinitionLink> {
    type Value = GraphDefinitionLink;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<GraphDefinitionLink>);
        impl<'de> serde::de::Visitor<'de> for Visitor<'_> {
            type Value = GraphDefinitionLink;
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                ...
                V: serde::de::MapAccess<'de>,
            {
                ...
                let mut r#compartment: Option<
                    Vec<fhirbolt_model::r5::resources::GraphDefinitionLinkCompartment>,
                > = None;
                while let Some(map_access_key) = tri!(map_access.next_key()) {
                    match map_access_key {
                        Field::Id => {
                        ...
                        Field::Compartment => {
                            if self.0.from == crate::context::Format::Json {
                                ...
                                let _context : & mut DeserializationContext < Vec < fhirbolt_model :: r5 :: resources :: GraphDefinitionLinkCompartment >> = self . 0 . transmute () ;
                                r#compartment =
                                    Some(tri!(map_access.next_value_seed(&mut *_context)));
                            } else {
                                let vec = r#compartment.get_or_insert(Default::default());
                                let _context: &mut DeserializationContext<
                                    fhirbolt_model::r5::resources::GraphDefinitionLinkCompartment,
                                > = self.0.transmute();
                                vec.push(tri!(map_access.next_value_seed(&mut *_context)));
                            }
                        }
                ...
                Ok(GraphDefinitionLink {
                    ...
                    r#min,
                    r#max,
                    r#source_id: if self.0.config.mode
                        == crate::context::de::DeserializationMode::Lax
                    {
                        r#source_id.unwrap_or(Default::default())
                    } else {
                        tri!(r#source_id.ok_or(serde::de::Error::missing_field("sourceId")))
                    },
                    r#path,
                    r#slice_name,
...
impl<'de> serde::de::DeserializeSeed<'de>
    for &mut DeserializationContext<Box<GraphDefinitionLink>>
{
    type Value = Box<GraphDefinitionLink>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        self.transmute::<GraphDefinitionLink>()
            .deserialize(deserializer)
            .map(Box::new)
    }
}
impl<'de> serde::de::DeserializeSeed<'de>
    for &mut DeserializationContext<Vec<GraphDefinitionLink>>
{
    type Value = Vec<GraphDefinitionLink>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<Vec<GraphDefinitionLink>>);
        impl<'de> serde::de::Visitor<'de> for Visitor<'_> {
            type Value = Vec<GraphDefinitionLink>;
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_str("a sequence")
            ...
            {
                let mut values = Vec::new();
                let _context: &mut DeserializationContext<GraphDefinitionLink> = self.0.transmute();
                while let Some(value) = tri!(seq.next_element_seed(&mut *_context)) {
                    values.push(value);
                }
                Ok(values)
...
use fhirbolt_model::r5::resources::GraphDefinition;
impl crate::Resource for GraphDefinition {
    const FHIR_RELEASE: fhirbolt_shared::FhirRelease = fhirbolt_shared::FhirReleases::R5;
}
impl serde::ser::Serialize for SerializationContext<&GraphDefinition> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
        ...
        fn missing_field_error<T, E: serde::ser::Error>(field: &str) -> Result<T, E> {
            Err(E::custom(format!(
                "missing required field `{}.{}`",
                "GraphDefinition", field
            )))
        }
        let mut state = tri!(serializer.serialize_map(None));
        tri!(state.serialize_entry("resourceType", "GraphDefinition"));
        if self.output == crate::context::Format::Json {
        ...
        {
            use fhirbolt_model::r5::resources::GraphDefinitionVersionAlgorithm as _Enum;
        ...
        if !self.value.r#node.is_empty() {
            tri!(self.with_context(&self.value.r#node, |ctx| state.serialize_entry("node", ctx)));
        }
...
impl serde::ser::Serialize for SerializationContext<&Box<GraphDefinition>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl serde::ser::Serialize for SerializationContext<&Vec<GraphDefinition>> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::ser::Serializer,
    {
...
impl<'de> serde::de::DeserializeSeed<'de> for DeserializationContext<GraphDefinition> {
...
impl<'de> serde::de::DeserializeSeed<'de> for &mut DeserializationContext<GraphDefinition> {
    type Value = GraphDefinition;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<GraphDefinition>);
        impl<'de> serde::de::Visitor<'de> for Visitor<'_> {
            type Value = GraphDefinition;
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                ...
                V: serde::de::MapAccess<'de>,
            {
                ...
                let mut r#version_algorithm: Option<
                    fhirbolt_model::r5::resources::GraphDefinitionVersionAlgorithm,
                > = None;
                ...
                let mut r#start: Option<fhirbolt_model::r5::types::Id> = None;
                let mut r#node: Option<Vec<fhirbolt_model::r5::resources::GraphDefinitionNode>> =
                    None;
                let mut r#link: Option<Vec<fhirbolt_model::r5::resources::GraphDefinitionLink>> =
                    None;
                while let Some(map_access_key) = tri!(map_access.next_key()) {
                    match map_access_key {
                        Field::ResourceType => {
                            let value: std::borrow::Cow<str> = tri!(map_access.next_value());
                            if value != "GraphDefinition" {
                        ...
                        Field::Node => {
                            if self.0.from == crate::context::Format::Json {
                                ...
                                let _context: &mut DeserializationContext<
                                    Vec<fhirbolt_model::r5::resources::GraphDefinitionNode>,
                                > = self.0.transmute();
                                r#node = Some(tri!(map_access.next_value_seed(&mut *_context)));
                            } else {
                                let vec = r#node.get_or_insert(Default::default());
                                let _context: &mut DeserializationContext<
                                    fhirbolt_model::r5::resources::GraphDefinitionNode,
                                > = self.0.transmute();
                                vec.push(tri!(map_access.next_value_seed(&mut *_context)));
                            }
                        }
...
impl<'de> serde::de::DeserializeSeed<'de> for &mut DeserializationContext<Box<GraphDefinition>> {
    type Value = Box<GraphDefinition>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        self.transmute::<GraphDefinition>()
            .deserialize(deserializer)
            .map(Box::new)
    }
}
impl<'de> serde::de::DeserializeSeed<'de> for &mut DeserializationContext<Vec<GraphDefinition>> {
    type Value = Vec<GraphDefinition>;
    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct Visitor<'a>(&'a mut DeserializationContext<Vec<GraphDefinition>>);
        impl<'de> serde::de::Visitor<'de> for Visitor<'_> {
            type Value = Vec<GraphDefinition>;
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_str("a sequence")
            ...
            {
                let mut values = Vec::new();
                let _context: &mut DeserializationContext<GraphDefinition> = self.0.transmute();
                while let Some(value) = tri!(seq.next_element_seed(&mut *_context)) {
                    values.push(value);
                }
                Ok(values)
