...
impl serde::ser::Serialize for SerializationContext<&NutritionIntakeConsumedItem> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        if self.value.r#nutrition_product.id.as_deref() == Some("$invalid") {
            ...
            tri!(
                self.with_context(&self.value.r#nutrition_product, |ctx| state
                    .serialize_entry("nutritionProduct", ctx))
            );
        }
        ...
        if let Some(some) = self.value.r#rate.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("rate", ctx)));
        }
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#not_consumed.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("notConsumed", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_notConsumed", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#not_consumed.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("notConsumed", ctx)));
        }
        if let Some(some) = self.value.r#not_consumed_reason.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("notConsumedReason", ctx)));
        }
        state.end()
    }
}
                    ...
                    match map_access_key {
                        ...
                        Field::Rate => {
                            ...
                            let _context: &mut DeserializationContext<
                                Box<fhirbolt_model::r5::types::Quantity>,
                            > = self.0.transmute();
                            r#rate = Some(tri!(map_access.next_value_seed(&mut *_context)));
                        }
                        Field::NotConsumed => {
                            if self.0.from == crate::context::Format::Json {
                ...
                V: serde::de::MapAccess<'de>,
            {
                ...
                Ok(NutritionIntakeIngredientLabel {
                        ...
                        == crate::context::de::DeserializationMode::Lax
                    {
                        ...
                        tri!(r#nutrient.ok_or(serde::de::Error::missing_field("nutrient")))
                    },
...
impl serde::ser::Serialize for SerializationContext<&NutritionIntake> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        let mut state = tri!(serializer.serialize_map(None));
        tri!(state.serialize_entry("resourceType", "NutritionIntake"));
        if self.output == crate::context::Format::Json {
        ...
        if !self.value.r#consumed_item.is_empty() {
            tri!(self.with_context(&self.value.r#consumed_item, |ctx| state
                .serialize_entry("consumedItem", ctx)));
        }
        if !self.value.r#ingredient_label.is_empty() {
            tri!(
                self.with_context(&self.value.r#ingredient_label, |ctx| state
                    .serialize_entry("ingredientLabel", ctx))
            );
        }
