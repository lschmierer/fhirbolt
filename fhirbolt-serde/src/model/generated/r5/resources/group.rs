        ...
        S: serde::ser::Serializer,
    {
        ...
        if self.output == crate::context::Format::Json {
            ...
            if let Some(some) = self.value.r#membership.value.as_ref().map(Ok) {
                tri!(state.serialize_entry("membership", &some?));
            }
            if self.value.r#membership.id.is_some() || !self.value.r#membership.extension.is_empty()
            {
                ...
                tri!(self.with_context(&primitive_element, |ctx| state
                    .serialize_entry("_membership", ctx)));
            }
        } else if self.value.r#membership.id.as_deref() == Some("$invalid") {
            return missing_field_error("membership");
        } else {
            tri!(self.with_context(&self.value.r#membership, |ctx| state
                .serialize_entry("membership", ctx)));
        }
                ...
                V: serde::de::MapAccess<'de>,
            {
                ...
                Ok(Group {
                        ...
                        == crate::context::de::DeserializationMode::Lax
                    {
                        ...
                        tri!(r#membership.ok_or(serde::de::Error::missing_field("membership")))
                    },
                    r#code,
                    r#name,
                    r#description,
