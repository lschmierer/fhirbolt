...
impl serde::ser::Serialize for SerializationContext<&DeviceUsage> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        let mut state = tri!(serializer.serialize_map(None));
        tri!(state.serialize_entry("resourceType", "DeviceUsage"));
        ...
        if let Some(some) = self.value.r#usage_status.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("usageStatus", ctx)));
        }
        if !self.value.r#usage_reason.is_empty() {
            tri!(self.with_context(&self.value.r#usage_reason, |ctx| state
                .serialize_entry("usageReason", ctx)));
        }
                    ...
                    match map_access_key {
                        ...
                        Field::UsageStatus => {
                            ...
                            let _context: &mut DeserializationContext<
                                Box<fhirbolt_model::r5::types::CodeableConcept>,
                            > = self.0.transmute();
                            r#usage_status = Some(tri!(map_access.next_value_seed(&mut *_context)));
                        }
                        Field::UsageReason => {
                            if self.0.from == crate::context::Format::Json {
