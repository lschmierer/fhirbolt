        ...
        S: serde::ser::Serializer,
    {
        ...
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#assurance.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("assurance", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    use super::super::serde_helpers::PrimitiveElement;
                    let primitive_element = PrimitiveElement {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_assurance", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#assurance.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("assurance", ctx)));
        }
        state.end()
    }
}
...
impl serde::ser::Serialize for SerializationContext<&Person> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        let mut state = tri!(serializer.serialize_map(None));
        tri!(state.serialize_entry("resourceType", "Person"));
        if self.output == crate::context::Format::Json {
