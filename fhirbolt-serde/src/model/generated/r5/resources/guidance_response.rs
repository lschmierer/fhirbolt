...
use fhirbolt_model::r5::resources::GuidanceResponse;
...
impl serde::ser::Serialize for SerializationContext<&GuidanceResponse> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        tri!(state.serialize_entry("resourceType", "GuidanceResponse"));
        ...
        if let Some(some) = self.value.r#request_identifier.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("requestIdentifier", ctx)));
        }
        ...
        {
            use fhirbolt_model::r5::resources::GuidanceResponseModule as _Enum;
            match self.value.r#module {
                _Enum::Uri(ref value) => {
                    if self.output == crate::context::Format::Json {
                        if let Some(some) = value.value.as_ref().map(Ok) {
                            tri!(state.serialize_entry("moduleUri", &some?));
                        }
                        if value.id.is_some() || !value.extension.is_empty() {
                            ...
                            tri!(self.with_context(&primitive_element, |ctx| state
                                .serialize_entry("_moduleUri", ctx)));
                        }
                    } else {
                        tri!(
                ...
                _Enum::Canonical(ref value) => {
                    if self.output == crate::context::Format::Json {
                        if let Some(some) = value.value.as_ref().map(Ok) {
                            tri!(state.serialize_entry("moduleCanonical", &some?));
                        }
                        if value.id.is_some() || !value.extension.is_empty() {
                            use super::super::serde_helpers::PrimitiveElement;
                            let primitive_element = PrimitiveElement {
                            ...
                            tri!(self.with_context(&primitive_element, |ctx| state
                                .serialize_entry("_moduleCanonical", ctx)));
                        }
                    } else {
                        tri!(self.with_context(value, |ctx| state
                            .serialize_entry("moduleCanonical", ctx)));
                    }
                }
                _Enum::CodeableConcept(ref value) => {
                    tri!(self.with_context(value, |ctx| state
                        .serialize_entry("moduleCodeableConcept", ctx)));
                }
                _Enum::Invalid => {
        ...
        if self.output == crate::context::Format::Json {
            ...
            tri!(self.with_context(some, |ctx| state.serialize_entry("occurrenceDateTime", ctx)));
        }
        ...
        if let Some(some) = self.value.r#evaluation_message.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("evaluationMessage", ctx)));
        }
        if let Some(some) = self.value.r#output_parameters.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("outputParameters", ctx)));
        }
                ...
                V: serde::de::MapAccess<'de>,
            {
                ...
                while let Some(map_access_key) = tri!(map_access.next_key()) {
                    match map_access_key {
                        ...
                        Field::ModuleCodeableConcept => {
                            ...
                            > = self.0.transmute();
                            r#module = Some(_Enum::CodeableConcept(tri!(
                                map_access.next_value_seed(&mut *_context)
                            )));
                        }
                        Field::Status => {
                ...
                Ok(GuidanceResponse {
                    ...
                    r#module: if self.0.config.mode == crate::context::de::DeserializationMode::Lax
                    {
                        ...
                        tri!(r#module.ok_or(serde::de::Error::missing_field("module[x]")))
                    },
