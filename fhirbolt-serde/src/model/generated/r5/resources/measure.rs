...
impl serde::ser::Serialize for SerializationContext<&MeasureGroupPopulation> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        if let Some(some) = self.value.r#group_definition.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("groupDefinition", ctx)));
        }
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#input_population_id.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("inputPopulationId", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    use super::super::serde_helpers::PrimitiveElement;
                    let primitive_element = PrimitiveElement {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_inputPopulationId", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#input_population_id.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("inputPopulationId", ctx)));
        }
        if let Some(some) = self.value.r#aggregate_method.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("aggregateMethod", ctx)));
        }
        state.end()
    }
}
        ...
        S: serde::ser::Serializer,
    {
        ...
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#basis.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("basis", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    use super::super::serde_helpers::PrimitiveElement;
                    let primitive_element = PrimitiveElement {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_basis", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#basis.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("basis", ctx)));
        }
        ...
        if let Some(some) = self.value.r#scoring_unit.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("scoringUnit", ctx)));
        }
        if self.output == crate::context::Format::Json {
                    ...
                    match map_access_key {
                        ...
                        Field::ScoringUnit => {
                            ...
                            let _context: &mut DeserializationContext<
                                Box<fhirbolt_model::r5::types::CodeableConcept>,
                            > = self.0.transmute();
                            r#scoring_unit = Some(tri!(map_access.next_value_seed(&mut *_context)));
                        }
