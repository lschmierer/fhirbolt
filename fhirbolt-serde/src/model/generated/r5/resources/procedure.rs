...
impl serde::ser::Serialize for SerializationContext<&Procedure> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        }
        {
            ...
            if let Some(some) = self.value.r#occurrence.as_ref() {
                match some {
                    ...
                    _Enum::Age(ref value) => {
                        tri!(self.with_context(value, |ctx| state
                            .serialize_entry("occurrenceAge", ctx)));
                    }
                    _Enum::Range(ref value) => {
                        tri!(self.with_context(value, |ctx| state
                            .serialize_entry("occurrenceRange", ctx)));
                    }
        ...
        if !self.value.r#used.is_empty() {
            tri!(self.with_context(&self.value.r#used, |ctx| state.serialize_entry("used", ctx)));
        }
                ...
                while let Some(map_access_key) = tri!(map_access.next_key()) {
                    match map_access_key {
                        ...
                        Field::OccurrenceRange => {
                            ...
                            > = self.0.transmute();
                            r#occurrence = Some(_Enum::Range(tri!(
                                map_access.next_value_seed(&mut *_context)
                            )));
                        }
                        ...
                        Field::Used => {
                            if self.0.from == crate::context::Format::Json {
                                ...
                                let _context: &mut DeserializationContext<
                                    Vec<fhirbolt_model::r5::types::CodeableReference>,
                                > = self.0.transmute();
                                r#used = Some(tri!(map_access.next_value_seed(&mut *_context)));
                            } else {
