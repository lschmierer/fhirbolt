        ...
        if self.output == crate::context::Format::Json {
            if !self.value.r#scope.is_empty() {
                ...
                if values.iter().any(|v| v.is_some()) {
                    tri!(state.serialize_entry("scope", &values));
                }
        ...
        if self.output == crate::context::Format::Json {
            if !self.value.r#allowed_type.is_empty() {
                let values = tri!(self
                    ...
                    .collect::<Result<Vec<_>, _>>());
                if values.iter().any(|v| v.is_some()) {
                    tri!(state.serialize_entry("allowedType", &values));
                }
                let requires_elements = self
                    .value
                    .r#allowed_type
                ...
                if requires_elements {
                    ...
                    let primitive_elements: Vec<_> = self
                        ...
                        .collect();
                    tri!(self.with_context(&primitive_elements, |ctx| state
                        .serialize_entry("_allowedType", ctx)));
                }
            }
        } else if !self.value.r#allowed_type.is_empty() {
            tri!(self.with_context(&self.value.r#allowed_type, |ctx| state
                .serialize_entry("allowedType", ctx)));
        }
        if self.output == crate::context::Format::Json {
