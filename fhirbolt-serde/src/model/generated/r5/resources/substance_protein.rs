...
use fhirbolt_model::r5::resources::SubstanceProteinSubunit;
        ...
        S: serde::ser::Serializer,
    {
        ...
        if let Some(some) = self.value.r#n_terminal_modification_id.as_ref() {
            tri!(self.with_context(some, |ctx| state
                .serialize_entry("nTerminalModificationId", ctx)));
        }
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#n_terminal_modification.as_ref() {
                if let Some(some) = some.value.as_ref().map(Ok) {
                    tri!(state.serialize_entry("nTerminalModification", &some?));
                }
                if some.id.is_some() || !some.extension.is_empty() {
                    use super::super::serde_helpers::PrimitiveElement;
                    let primitive_element = PrimitiveElement {
        ...
        if self.output == crate::context::Format::Json {
            if let Some(some) = self.value.r#c_terminal_modification.as_ref() {
                ...
                if some.id.is_some() || !some.extension.is_empty() {
                    ...
                    tri!(self.with_context(&primitive_element, |ctx| state
                        .serialize_entry("_cTerminalModification", ctx)));
                }
            }
        } else if let Some(some) = self.value.r#c_terminal_modification.as_ref() {
            tri!(self.with_context(some, |ctx| state
                .serialize_entry("cTerminalModification", ctx)));
        }
        state.end()
    }
}
...
use fhirbolt_model::r5::resources::SubstanceProtein;
...
impl serde::ser::Serialize for SerializationContext<&SubstanceProtein> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        let mut state = tri!(serializer.serialize_map(None));
        tri!(state.serialize_entry("resourceType", "SubstanceProtein"));
        if self.output == crate::context::Format::Json {
        ...
        if self.output == crate::context::Format::Json {
            if !self.value.r#disulfide_linkage.is_empty() {
                ...
                if values.iter().any(|v| v.is_some()) {
                    tri!(state.serialize_entry("disulfideLinkage", &values));
                }
                ...
                if requires_elements {
                    ...
                    tri!(self.with_context(&primitive_elements, |ctx| state
                        .serialize_entry("_disulfideLinkage", ctx)));
                }
            }
        } else if !self.value.r#disulfide_linkage.is_empty() {
            tri!(
                self.with_context(&self.value.r#disulfide_linkage, |ctx| state
                    .serialize_entry("disulfideLinkage", ctx))
            );
        }
