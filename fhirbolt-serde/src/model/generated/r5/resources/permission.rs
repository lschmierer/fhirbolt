...
impl serde::ser::Serialize for SerializationContext<&Permission> {
        ...
        S: serde::ser::Serializer,
    {
        ...
        let mut state = tri!(serializer.serialize_map(None));
        tri!(state.serialize_entry("resourceType", "Permission"));
        if self.output == crate::context::Format::Json {
        ...
        if self.output == crate::context::Format::Json {
            if !self.value.r#date.is_empty() {
                ...
                if values.iter().any(|v| v.is_some()) {
                    tri!(state.serialize_entry("date", &values));
                }
        ...
        if let Some(some) = self.value.r#validity.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("validity", ctx)));
        }
        if let Some(some) = self.value.r#justification.as_ref() {
            tri!(self.with_context(some, |ctx| state.serialize_entry("justification", ctx)));
        }
        if self.output == crate::context::Format::Json {
            ...
            if let Some(some) = self.value.r#combining.value.as_ref().map(Ok) {
                tri!(state.serialize_entry("combining", &some?));
            }
            if self.value.r#combining.id.is_some() || !self.value.r#combining.extension.is_empty() {
                ...
                tri!(self.with_context(&primitive_element, |ctx| state
                    .serialize_entry("_combining", ctx)));
            }
        } else if self.value.r#combining.id.as_deref() == Some("$invalid") {
            return missing_field_error("combining");
        } else {
            tri!(self.with_context(&self.value.r#combining, |ctx| state
                .serialize_entry("combining", ctx)));
        }
                ...
                V: serde::de::MapAccess<'de>,
            {
                ...
                while let Some(map_access_key) = tri!(map_access.next_key()) {
                    match map_access_key {
                        ...
                        Field::Validity => {
                            ...
                            > = self.0.transmute();
                            r#validity = Some(tri!(map_access.next_value_seed(&mut *_context)));
                        }
                        Field::Justification => {
                ...
                Ok(Permission {
                        ...
                        == crate::context::de::DeserializationMode::Lax
                    {
                        ...
                        tri!(r#combining.ok_or(serde::de::Error::missing_field("combining")))
                    },
