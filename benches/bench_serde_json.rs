...
pub fn criterion_benchmark(c: &mut Criterion) {
    let mut examples = examples(FhirRelease::R4, JsonOrXml::Json);

    let valuesets_bytes = examples.get("valuesets.json").read_to_vec();
    ...
    c.bench_function("read valuesets", |b| {
        b.iter(|| {
            black_box(
                fhirbolt::json::from_slice::<model::r4::resources::Bundle>(&valuesets_bytes, None)
                    .unwrap(),
            )
        })
    })
    .bench_function("write valuesets", |b| {
        b.iter(|| black_box(fhirbolt::json::to_vec(&valuesets_bundle)))
    });
}
